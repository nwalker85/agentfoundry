...
class AgentLoader:
    ...
    async def load_agent(self, yaml_config: AgentYAML) -> AgentInstance:
        ...
        try:
            ...
            logger.info(
                f"Successfully loaded agent {agent_id} "
                f"with {len(yaml_config.spec.workflow.nodes)} nodes"
            )
    ...
    async def _build_graph(self, yaml_config: AgentYAML) -> Any:
        ...
        logger.info(
            f"Compiled graph for {yaml_config.metadata.id} "
    ...
    async def _import_handler(self, handler_path: str) -> Callable:
        ...
        try:
            ...
            if len(parts) != 3:
                raise ValueError(
                    f"Handler path must be module.Class.method, got: {handler_path}"
                )
            ...
            raise ImportError(
                f"Could not import handler '{handler_path}': {e}"
            ) from e
        ...
        try:
            ...
            for node_config in yaml_config.spec.workflow.nodes:
                try:
                    ...
                    result.add_error(
                        f"Node '{node_config.id}' handler import failed: {e}"
                    )
            ...
            for node_config in yaml_config.spec.workflow.nodes:
                if not node_config.next:
                    result.add_warning(
                        f"Node '{node_config.id}' has no outgoing edges "
                        "(will hang if reached)"
                    )
