...
class MarshalAgent:
    ...
    def __init__(
        ...
        self.file_watcher: Optional[AgentFileWatcher] = None
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Marshal Agent started successfully "
                f"({await self.registry.count()} agents loaded)"
            )
    ...
    async def _load_all_agents(self):
        ...
        logger.info(
            f"Loaded {loaded_count}/{len(yaml_files)} agents successfully"
        )
    ...
    async def _load_agent_file(self, yaml_path: Path):
        ...
        try:
            ...
            if not validation_result.valid:
                ...
                logger.error(
                    f"❌ Validation failed for {agent_id}: {error_msg}"
                )
            ...
            logger.info(
                f"✅ Loaded {agent_id} v{agent_config.metadata.version} "
                f"({len(agent_config.spec.workflow.nodes)} nodes)"
            )
            ...
            logger.error(
                f"Failed to load {yaml_path.name}: {len(result.errors)} validation error(s)"
            )
            raise
    ...
    async def _handle_file_change(
        ...
        try:
            ...
            logger.error(
                f"Error handling {change_type.value} for {file_path.name}: {e}",
                exc_info=True
            )
    ...
    async def _handle_agent_modified(self, file_path: Path):
        ...
        try:
            ...
            logger.info(
                f"🔄 Reloaded {agent_id} v{agent_config.metadata.version}"
            )
            ...
            logger.error(
                f"Failed to reload {file_path.name}: {len(result.errors)} validation error(s)"
            )
