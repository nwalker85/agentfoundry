...
class SupervisorAgent:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_conditional_edges(
            "analyze_request", self._route_worker, {"pm_agent": "pm_agent", "FINISH": "compile_response"}
        )
    ...
    async def _load_context_node(self, state: AgentState) -> AgentState:
        ...
        enriched_context = await self.context_agent.process(
            session_id=session_id, user_id=user_id, current_input=current_input
        )
...
Your response (one word only):"""
        ...
        response = await self.llm.ainvoke([SystemMessage(content=system_prompt.format(message=user_input))])
        ...
        return {**state, "next_agent": next_agent, "routed_to": [next_agent]}
    ...
    async def _pm_agent_node(self, state: AgentState) -> AgentState:
        ...
        return {**state, "worker_responses": worker_responses}
    ...
    async def process(self, io_state: IOState) -> str:
        ...
        agent_state = AgentState(
            ...
            session_context={"session_id": io_state.get("session_id", "default_session")},
            user_context={"user_id": io_state.get("user_id", "default_user")},
        )
    ...
    async def execute_parallel_workers(self, worker_agents: list[Any], state: dict[str, Any]) -> dict[str, Any]:
        """
    ...
    async def execute_with_timeout(
        self, node_func: Callable, state: dict[str, Any], timeout_seconds: float = 60.0
    ) -> Any:
        """
    ...
    async def enforce_recursion_limit(self, current_depth: int, max_depth: int) -> bool:
        """
    ...
    async def execute_with_streaming(self, state: dict[str, Any], stream_mode: bool = False) -> Any:
        """
