...
class WorkflowConfig(BaseModel):
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        if self.entry_point not in node_ids:
            raise ValueError(
                f"Entry point '{self.entry_point}' not found in nodes: {node_ids}"
            )
        ...
        for node in self.nodes:
            for next_id in node.next:
                if next_id not in node_ids and next_id != "END":
                    raise ValueError(
                        f"Node '{node.id}' references unknown next node: {next_id}"
                    )
...
class AgentStatus(BaseModel):
    ...
    phase: str = Field(
        default="alpha",
        pattern=r'^(alpha|beta|production|deprecated)$'
    )
    state: str = Field(
        default="inactive",
...
class AgentYAML(BaseModel):
    ...
    apiVersion: str = Field(
        ...,
        pattern=r'^engineering-dept/v\d+$',
        description="API version (e.g., engineering-dept/v1)"
    )
    kind: str = Field(
        ...,
        pattern=r'^Agent$',
