...
class CoherenceAgent:
    ...
    async def _resolve_conflicts(
        self,
        deduplicated_responses: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
...
Provide a single, coherent, natural response:"""
            ...
            response = await self.llm.ainvoke([
                SystemMessage(content=system_prompt.format(responses=responses_text))
            ])
    ...
    async def validate_coherence(self, compiled_response: str) -> Dict[str, Any]:
        ...
        validation = {
            ...
            "sentence_count": compiled_response.count('.') + compiled_response.count('!') + compiled_response.count('?'),
