...
class ObservabilityAgent:
    ...
    async def start_trace(self, agent_id: str, execution_id: str, metadata: dict[str, Any] | None = None) -> str:
        """
    ...
    async def create_state_snapshot(self, trace_id: str, node_id: str, state: dict[str, Any]):
        """
        ...
        snapshot = {"node_id": node_id, "timestamp": datetime.now().isoformat(), "state": self._sanitize_state(state)}
    ...
    async def _send_to_langsmith(self, trace_id: str, trace_data: dict[str, Any], summary: dict[str, Any]):
        """
    ...
    async def get_trace(self, trace_id: str) -> dict[str, Any] | None:
        ...
        if matching_traces:
            return {"trace_id": trace_id, "nodes": [asdict(t) for t in matching_traces], "status": "completed"}

        return None

    async def get_agent_traces(self, agent_id: str, limit: int = 100) -> list[dict[str, Any]]:
        """
    ...
    async def get_metrics(self, agent_id: str, time_window_hours: int = 24) -> AgentMetrics:
        """
        ...
        recent_metrics = [m for m in metrics if datetime.fromisoformat(m["timestamp"]).timestamp() > cutoff]
    ...
    async def get_execution_playback(self, trace_id: str) -> list[dict[str, Any]] | None:
        """
    ...
    async def detect_performance_degradation(self, agent_id: str, threshold_increase: float = 1.5) -> dict[str, Any]:
        """
    ...
    async def export_traces(self, agent_id: str | None = None, format: str = "json") -> str:
        """
