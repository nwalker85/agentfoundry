...
class AgentFileWatcher:
    ...
    def __init__(self, watch_path: Path, on_change: Callable[[FileChangeType, Path], None]):
    ...
    async def _handle_change(self, change_type: FileChangeType, file_path: Path):
        ...
        try:
            ...
            logger.error(f"Error handling {change_type.value} for {file_path}: {e}", exc_info=True)
