...
from .yaml_validator import AgentYAML, AgentStatus
...
class AgentRegistry:
    ...
    async def reload(self, agent_id: str, new_instance: AgentInstance) -> None:
        ...
        async with self._lock:
            ...
            if old_instance:
                ...
                logger.info(
                    f"Reloading agent {agent_id}: "
                    f"{old_instance.metadata.version} → {new_instance.metadata.version}"
                )
    ...
    async def get_healthy_count(self) -> int:
        ...
        async with self._lock:
            return sum(
                1 for instance in self._agents.values()
