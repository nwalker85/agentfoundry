...
> **Note:** This document describes the original **single‑EC2 + docker‑compose**
> deployment path and is kept for historical reference.  
> The **current, supported** AWS deployment for Agent Foundry uses **ECR + ECS
> Fargate + ALB** via Terraform in `infra/`.  
...
See [deploy/AWS_DEPLOYMENT_GUIDE.md](deploy/AWS_DEPLOYMENT_GUIDE.md) for
complete details.
