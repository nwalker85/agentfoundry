...
- Changed `LIVEKIT_URL` from `ws://host.docker.internal:7880` to
  `ws://livekit:7880`
...
**Root Cause:** Docker container built before LangChain 1.0 upgrade dependencies
were finalized. The `requirements.txt` includes `langchain-anthropic==1.0.4` but
the container image wasn't rebuilt.
