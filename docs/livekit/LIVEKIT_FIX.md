...
The `/api/voice/session` endpoint was returning "Internal Server Error" when
called.

## Root Cause

The `LiveKitService` methods were declared as `async` and being called with
`await`, but the LiveKit Python SDK's `RoomService` methods are **synchronous**,
not asynchronous. This caused the await to fail.
...
The endpoint functions themselves remain `async` for FastAPI compatibility, but
the LiveKit service calls are synchronous.

## Testing

### Prerequisites

Install LiveKit dependencies first (see
[LIVEKIT_INSTALL.md](LIVEKIT_INSTALL.md)):
...
- **Blocking**: LiveKit calls block briefly but are fast enough for HTTP
  responses
...
1. Running in thread pool executor:
   `await run_in_executor(None, service.create_session, ...)`
...
1. `backend/livekit_service.py` - Converted to sync methods + fixed
   RoomServiceClient import
