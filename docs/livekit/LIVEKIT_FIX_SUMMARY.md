...
Our LiveKit voice agent implementation was using **deprecated API patterns** and
**missing critical WebRTC connection steps**, preventing the agent from joining
rooms with users.

### Specific Issues:

1. **Old SDK Pattern**: Using `VoiceAssistant` class (deprecated) instead of
   modern `Agent` + `AgentSession`
2. **Missing WebRTC Connection**: No `await ctx.connect()` call
3. **Overcomplicated LLM Wrapper**: Custom `IOAgentLLMWrapper` class trying to
   bridge incompatible interfaces
4. **Unclear Tool Integration**: No clear pattern for function calling

## Root Cause

We were following pre-2024 LiveKit examples that used the `VoiceAssistant` API.
LiveKit has since modernized to the `Agent` + `AgentSession` pattern, which is:
...
See `docs/LIVEKIT_TESTING_GUIDE.md` for comprehensive testing procedures and
troubleshooting.
...
**None** - This is a fix to existing (broken) functionality. The API surface
remains the same:
