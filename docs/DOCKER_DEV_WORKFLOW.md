...
**Purpose:** This doc is your **day‑to‑day local dev guide** for running the
Agent Foundry stack with Docker + Next.js, including hot reload behavior and
when you actually need to rebuild.

For a higher‑level overview and AWS deployment, see `START_HERE.md` and
`DEPLOYMENT.md`.
...
- Uses `docker compose` (plus `docker-compose.dev.yml` if present) to start
  services
...
Fix the error and save – the container will auto‑restart when the code is valid
again.
...
For architecture and deeper troubleshooting, see `LIVEKIT_SETUP.md` and
