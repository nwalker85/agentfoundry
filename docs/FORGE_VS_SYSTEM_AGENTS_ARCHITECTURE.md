...
After reviewing the FORGE_LANGGRAPH_GAP_ANALYSIS.md, it's clear that many "missing features" are actually **runtime concerns** that should be handled by **system-level agents**, not the Forge UI.
...
- **Why System Agent**: Runtime concern, needs to work across all customer agents
...
These features need both design-time configuration (Forge) and runtime execution (agents):
...
- **How they work together**:
  - Forge defines: "On error in node X, retry 3 times, then route to fallback_node"
...
**Key Insight**: About **60% of the "Forge gaps"** are actually **system agent concerns**!
...
This architecture properly separates **design concerns** (Forge) from **runtime concerns** (System Agents), making both more maintainable and scalable.
