...
This guide explains how to integrate and use the 7 platform system agents in Agent Foundry. These agents form the runtime infrastructure that enables all customer worker agents to function reliably.
...
2. Check for degradation: `await observability_agent.detect_performance_degradation(agent_id)`
3. Review traces: `await observability_agent.get_agent_traces(agent_id, limit=50)`

### Rate Limits Hit

1. Check current limits: `await governance_agent.get_policies()`
2. Update policies: `await governance_agent.update_policy({'max_cost_per_request': 2.0})`
...
3. Implement session cleanup: `await context_agent.clear_session(old_session_id)`
