...
export default function SystemAgentsPage() {
  ...
  const saveAgentConfig = async (agentId: string) => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: `${agents[agentId].metadata.name} configuration saved successfully` });
  ...
  return (
    <div className="flex flex-col h-screen bg-bg-0">
      {/* Header */}
      <div className="h-14 border-b border-white/10 bg-bg-1 flex items-center px-6">
        <Settings className="w-5 h-5 text-blue-500 mr-3" />
        <h1 className="text-lg font-semibold text-fg-0">System Agents Configuration</h1>
        <div className="ml-auto flex items-center gap-2">
          <Badge variant="outline" className="border-green-500/30 text-green-400">
            {Object.values(agents).filter(a => a.status.state === 'active').length} Active
          </Badge>
          <Button variant="outline" size="sm" onClick={loadAgentConfigs}>
      ...
      {/* Status Messages */}
      {message && (
        <Alert className={`mx-6 mt-4 ${message.type === 'success' ? 'border-green-500/30 bg-green-500/10' : 'border-red-500/30 bg-red-500/10'}`}>
          ...
          <div className="p-4">
            ...
            <div className="mt-6 p-3 rounded-lg bg-blue-500/10 border border-blue-500/30">
              ...
              <p className="text-xs text-blue-300">
                Tier 0 agents ARE the platform. Tier 1 agents are essential for production readiness (error handling, monitoring, security).
              </p>
            </div>
          </div>
        </div>
              ...
              <Tabs defaultValue="general" className="space-y-4">
                ...
                <TabsContent value="general" className="space-y-4">
                  <Card className="p-6 bg-bg-1 border-white/10">
                    ...
                    <div className="space-y-4">
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Version</label>
                        <Input
                          value={currentAgent.metadata.version}
                          onChange={e => updateAgentConfig(selectedAgent, {
                            ...
                            <Switch
                              ...
                              onCheckedChange={checked =>
                                updateAgentConfig(selectedAgent, {
                                  status: { ...currentAgent.status, state: checked ? 'active' : 'inactive' }
                                })
                              }
                            />
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Deployment Phase</label>
                        <select
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Capabilities</label>
                        <div className="flex flex-wrap gap-2">
                          {currentAgent.spec.capabilities.map(cap => (
                            <Badge key={cap} className="bg-blue-500/20 text-blue-300 border-blue-500/30">
                              {cap.replace(/_/g, ' ')}
                            </Badge>
                          ))}
                        </div>
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Workflow Entry Point</label>
                        <Input
                          value={currentAgent.spec.workflow.entry_point}
                          onChange={e => updateAgentConfig(selectedAgent, {
                            spec: {
                              ...currentAgent.spec,
                              workflow: { ...currentAgent.spec.workflow, entry_point: e.target.value }
                            }
                          })}
                          className="bg-bg-2 border-white/10"
                        />
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Recursion Limit</label>
                        <Input
                          type="number"
                          ...
                          onChange={e => updateAgentConfig(selectedAgent, {
                            spec: {
                              ...currentAgent.spec,
                              workflow: { ...currentAgent.spec.workflow, recursion_limit: parseInt(e.target.value) }
                            }
                          })}
                          className="bg-bg-2 border-white/10"
                        />
                ...
                <TabsContent value="resources" className="space-y-4">
                  <Card className="p-6 bg-bg-1 border-white/10">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Max Tokens Per Request</label>
                        <Input
                          type="number"
                      ...
                      {currentAgent.spec.integrations.map((integration, index) => (
                        <Card key={index} className="p-4 bg-bg-2 border-white/10">
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <Badge
                              variant="outline"
                              className={integration.required ? 'border-red-500/30 text-red-400' : 'border-yellow-500/30 text-yellow-400'}
                            >
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex flex-wrap gap-1">
                              {integration.operations.map(op => (
                                <Badge key={op} variant="outline" className="text-xs border-white/10 text-fg-2">
                                  {op}
                                </Badge>
                              ))}
                            </div>
                ...
                <TabsContent value="observability" className="space-y-4">
                  <Card className="p-6 bg-bg-1 border-white/10">
                    ...
                    <div className="space-y-4">
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Trace Level</label>
                        <select
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <label className="text-sm font-medium text-fg-1 block">Metrics Enabled</label>
                          <p className="text-xs text-fg-3 mt-1">Collect performance and usage metrics</p>
                        </div>
                        <Switch
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Metrics Interval (seconds)</label>
                        <Input
                          type="number"
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Log Retention (days)</label>
                        <Input
                          type="number"
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          <label className="text-sm font-medium text-fg-1 block">Structured Logging</label>
                ...
                <TabsContent value="health" className="space-y-4">
                  <Card className="p-6 bg-bg-1 border-white/10">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <label className="text-sm font-medium text-fg-1 block">Health Checks Enabled</label>
                          <p className="text-xs text-fg-3 mt-1">Automatically monitor agent health</p>
                        </div>
                        <Switch
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Check Interval (seconds)</label>
                        <Input
                          type="number"
                      ...
                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Check Timeout (seconds)</label>
                        <Input
                          type="number"
                          ...
                          disabled={!currentAgent.spec.health_check.enabled}
                        />
                        <p className="text-xs text-fg-3 mt-1">Maximum time to wait for health check response</p>
                      </div>

                      <div>
                        <label className="text-sm font-medium text-fg-1 mb-1.5 block">Unhealthy Threshold</label>
                        <Input
                          type="number"
                          ...
                          disabled={!currentAgent.spec.health_check.enabled}
                        />
                        <p className="text-xs text-fg-3 mt-1">Failed checks before marking agent as unhealthy</p>
                      </div>

                      {/* Current Health Status */}
                      <Card className="p-4 bg-bg-2 border-white/10 mt-6">
                        <h4 className="text-sm font-medium text-fg-1 mb-3">Current Health Status</h4>
                        <div className="space-y-2">
                          <div className="flex items-center justify-between">
