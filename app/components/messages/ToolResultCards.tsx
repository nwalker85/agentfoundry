...
function StoryCard({ artifact, isUser }: ToolResultCardProps) {
  ...
  const priorityStyle =
    priorityColors[priority as keyof typeof priorityColors] || priorityColors.P3;
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span
            className={`font-semibold text-sm ${isUser ? 'text-blue-900 dark:text-blue-100' : 'text-gray-900 dark:text-gray-100'}`}
          >
            Story Created
          </span>
        </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div>
          <h4
            className={`font-semibold text-base leading-tight ${isUser ? 'text-blue-900 dark:text-blue-100' : 'text-gray-900 dark:text-gray-100'}`}
          >
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          {data?.epic && (
            <div className="flex items-start gap-2">
              <Target
                className={`w-4 h-4 mt-0.5 flex-shrink-0 ${isUser ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}
              />
              <div className="min-w-0">
                <div
                  className={`text-xs font-medium ${isUser ? 'text-blue-700 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400'}`}
                >
                  Epic
                </div>
                <div
                  className={`truncate ${isUser ? 'text-blue-900 dark:text-blue-100' : 'text-gray-900 dark:text-gray-100'}`}
                >
                  {data.epic}
                </div>
              </div>
        ...
        {data?.acceptance_criteria && data.acceptance_criteria.length > 0 && (
          ...
          >
            <div
              className={`text-xs font-medium mb-1 ${isUser ? 'text-blue-700 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400'}`}
            >
            ...
            <div className="space-y-1">
              {data.acceptance_criteria.slice(0, 2).map((ac: string, idx: number) => (
                <div
                  key={idx}
                  className={`text-xs flex items-start gap-2 ${isUser ? 'text-blue-800 dark:text-blue-200' : 'text-gray-600 dark:text-gray-300'}`}
                >
              ...
              {data.acceptance_criteria.length > 2 && (
                <div
                  className={`text-xs italic ${isUser ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}
                >
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          {data?.assignees && data.assignees.length > 0 && (
            <div className="flex items-start gap-2">
              <div
                className={`w-4 h-4 mt-0.5 flex-shrink-0 rounded-full ${isUser ? 'bg-blue-600 dark:bg-blue-400' : 'bg-gray-500 dark:bg-gray-400'}`}
              />
              <div className="min-w-0">
                <div
                  className={`text-xs font-medium ${isUser ? 'text-blue-700 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400'}`}
