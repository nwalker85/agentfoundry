...
from mcp.schemas import CreateIssueRequest, CreateIssueResponse
...
class GitHubTool:
    ...
    async def create_issue(self, request: CreateIssueRequest) -> CreateIssueResponse:
        ...
        body_parts.append("created_by: engineering-department-agent")
    ...
    async def create_pr(
        ...
        response = await self.client.post(
            ...
            json={"title": title, "body": pr_body, "head": head_branch, "base": base, "draft": draft},
        )
        response.raise_for_status()
        data = response.json()
    ...
    async def _find_issue_by_idempotency_key(self, key: str) -> CreateIssueResponse | None:
    ...
    async def _get_milestone_number(self, title: str) -> int | None:
        """Get milestone number by title."""

        response = await self.client.get(f"/repos/{self.owner}/{self.repo_name}/milestones", params={"state": "open"})
        ...
        create_response = await self.client.post(
            f"/repos/{self.owner}/{self.repo_name}/milestones", json={"title": title}
        )
