"""
PM Agent - LangGraph Workflow
Orchestrates project management operations with Ollama LLM
"""
...
"""
    ...
    async def breakdown_work(self, state: PMState) -> PMState:
        """Break down requirements into epic/stories/tasks"""
        user_input = state["user_input"]
        logger.info("Starting work breakdown")
        start = time.time()
        
        prompt = BREAKDOWN_PROMPT_TEMPLATE.format(user_input=user_input)
        ...
        try:
            ...
            for story_data in breakdown.get("stories", []):
                story = Story(
                    ...
                    tasks=[],
                    priority=TaskPriority[story_data.get("priority", "MEDIUM")],
            ...
            perf.track("breakdown_work", time.time() - start, stories=len(stories), tasks=len(tasks))
...
"""
            ...
            perf.track("track_progress", time.time() - start, 
                      in_progress=len(in_progress), blocked=len(blocked))
        else:
...
"""
    ...
    async def process_message(self, user_input: str, session_state: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            perf.track("process_message", time.time() - start, success=False)
            raise
    ...
    async def initialize(self):
        ...
        try:
            ...
            if result["success"]:
                ...
                perf.track("initialize", time.time() - start)
                return True
            else:
                logger.error(f"Database initialization failed: {result['error']}")
                perf.track("initialize", time.time() - start, success=False)
                return False
        except Exception as e:
