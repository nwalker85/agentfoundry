...
def merge_status_docs():
    ...
    content.append(f"*Merged from: Current State.md, PROJECT_PLAN.md, PROJECT_INVENTORY.md*\n\n")
    ...
    if project_inventory.exists():
        with open(project_inventory, 'r') as f:
...
def merge_test_docs():
    ...
    content.append(f"*Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}*\n")
    content.append(f"*Merged from: UAT_GUIDE.md, TEST_README.md, QUICK_TEST.md*\n\n")
